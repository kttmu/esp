...
def espnet(x, model):
    ...
    pred = conv_one_cross_one(concat3, 22)
    if model == "espnet_c":
        return pred
    else:
        deconv1 = tf.layers.conv2d_transpose(pred, 22, [2, 2], strides=(1, 1), padding='same')
        conv_1 = conv_one_cross_one(concat2, 22)
        concat4 = tf.concat((deconv1, conv_1), axis=-1)
        esp_3 = esp(concat4, 22)
        deconv2 = tf.layers.conv2d_transpose(esp_3, 22, [2, 2], strides=(1, 1), padding='same')
        conv_2 = conv_one_cross_one(concat1, 22)
        concat5 = tf.concat((deconv2, conv_2), axis=-1)
        conv_3 = conv_one_cross_one(concat5, 22)
        deconv3 = tf.layers.conv2d_transpose(conv_3, 22, [2, 2], strides=(1, 1), padding='same')
        return deconv3
